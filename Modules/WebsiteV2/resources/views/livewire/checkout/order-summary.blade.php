<div class="sticky top-24">
    <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 relative overflow-hidden">
        ...
        <h3 class="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
            ...
            Your order <span class="text-gray-400 text-sm font-normal">({{ $this->summary['items']->count() }} products)</span>
        </h3>
            ...
            @foreach ($this->summary['items'] as $item)
                <div class="flex gap-4 py-2 group">
                    <div class="w-16 h-16 bg-gray-50 rounded-lg border border-gray-200 overflow-hidden shrink-0 relative">
                        <img src="{{ $item->product->image_url ?? 'https://placehold.co/400x400' }}" class="w-full h-full object-cover" alt="{{ $item->product->title ?? 'Product' }}">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="mt-1">
                            <span class="font-bold text-gray-900 text-sm">{{ number_format($item->total) }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="mb-6 pt-6 border-t border-gray-100 border-dashed">
            <label class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 block">Coupon code</label>
            <div class="flex gap-2">
                <input type="text" wire:model="couponCode" wire:keydown.enter="applyCoupon" placeholder="Enter code..." class="flex-1 bg-gray-50 border border-gray-200 rounded-lg text-sm px-3 py-2.5 focus:ring-black focus:border-black transition">
                <button wire:click="applyCoupon" wire:loading.attr="disabled" class="px-4 py-2 bg-gray-200 text-gray-700 font-bold rounded-lg text-sm hover:bg-gray-300 transition">
        ...
        <div class="space-y-3 border-t border-gray-100 border-dashed pt-6">
            <div class="flex justify-between text-sm">
                ...
                <span class="font-medium text-gray-900">{{ number_format($this->summary['subtotal']) }}</span>
            </div>
            ...
            @if ($this->summary['discount'] > 0)
                <div class="flex justify-between items-center text-sm text-green-600 bg-green-50 p-2 rounded-lg">
                    <div class="flex flex-col">
                        ...
                        <button wire:click="removeCoupon" class="text-xs underline text-red-500 text-left mt-0.5 hover:text-red-700">Remove</button>
                    </div>
                    <span class="font-bold">-{{ number_format($this->summary['discount']) }}</span>
                </div>
            @endif
            ...
            <div class="border-t border-gray-100 pt-4 mt-2 flex justify-between items-end">
                ...
                <div class="text-right">
                    <span class="block text-2xl font-black text-blue-600 tracking-tight">{{ number_format($this->summary['total']) }}</span>
                    <span class="text-[10px] text-gray-400 font-medium">VAT included</span>
                </div>
            </div>
        </div>
