<div class="space-y-8">
    <div class="flex items-center justify-between">
        ...
        @if (! empty($selected_categories) || ! empty($price_range) || ! empty($search))
            <button wire:click="resetFilters" class="text-xs text-red-500 font-medium hover:underline" type="button">
                Clear all
            </button>
        @endif
    </div>
    ...
    <div x-data="{ open: true }" class="border-b border-gray-100 pb-6">
        <button @click="open = !open" class="flex items-center justify-between w-full py-2 text-left font-medium text-gray-900 hover:text-blue-600 transition" type="button">
        ...
        <div x-show="open" x-collapse class="mt-4 space-y-3">
            ...
            @empty
                <p class="text-sm text-gray-500">No product categories.</p>
            @endforelse
        </div>
    </div>
    ...
    <div x-data="{ open: true }" class="border-b border-gray-100 pb-6">
        ...
        <div x-show="open" x-collapse class="mt-4 space-y-3">
            <label class="flex items-center group cursor-pointer">
                ...
                <span class="ml-3 text-sm text-gray-600">Under 100k</span>
            </label>
            <label class="flex items-center group cursor-pointer">
            ...
            <label class="flex items-center group cursor-pointer">
                ...
                <span class="ml-3 text-sm text-gray-600">Over 500k</span>
            </label>
        </div>
    </div>
</div>
