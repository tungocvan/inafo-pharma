...
class="min-h-screen bg-gray-50 font-sans">
    ...
    <div class="bg-white border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <nav class="flex text-sm text-gray-500 mb-2">
                <a href="{{ route('website-v2.home') }}" class="hover:text-blue-600 transition">Home</a>
                <span class="mx-2">/</span>
                <span class="text-gray-900 font-medium">Products</span>
            </nav>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">All Products</h1>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div x-show="mobileFilterOpen" class="relative z-50 lg:hidden" role="dialog" aria-modal="true">
                <div x-show="mobileFilterOpen" x-transition.opacity class="fixed inset-0 bg-black bg-opacity-25" @click="mobileFilterOpen = false"></div>
                ...
                <div x-show="mobileFilterOpen"
                    ...
                    <div class="px-4">
                        @include('website-v2::livewire.products.partials.filters')
                    </div>
                </div>
            </div>
            ...
            <main class="w-full lg:w-3/4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 pb-6 border-b border-gray-200 gap-4">
                    ...
                    <span class="text-sm text-gray-500">Showing <span class="font-bold text-gray-900">{{ $products->count() }}</span> results</span>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="hidden sm:flex bg-white border border-gray-200 rounded-lg p-1 shadow-sm">
                            <button @click="viewMode = 'grid'" :class="{'bg-gray-100 text-blue-600': viewMode === 'grid', 'text-gray-400 hover:text-gray-600': viewMode !== 'grid'}" class="p-1.5 rounded transition-all" type="button">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                            </button>
                            <button @click="viewMode = 'list'" :class="{'bg-gray-100 text-blue-600': viewMode === 'list', 'text-gray-400 hover:text-gray-600': viewMode !== 'list'}" class="p-1.5 rounded transition-all" type="button">
                ...
                <div class="relative min-h-[400px]">
                    ...
                    @if ($products->isEmpty())
                        <div class="text-center py-16 bg-white rounded-xl border border-dashed border-gray-300">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Try changing filters or search terms.</p>
                            <button wire:click="resetFilters" class="mt-6 inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">
                                Clear filters
                            </button>
                        </div>
                    @else
                        <div :class="viewMode === 'grid' ? 'grid grid-cols-2 md:grid-cols-3 gap-6' : 'space-y-4'">
                            @foreach ($products as $product)
                                <div wire:key="website-v2-product-{{ $product->id }}"
                                    ...
                                    <div class="relative overflow-hidden bg-gray-100"
                                        ...
                                        <a href="{{ route('website-v2.product.detail', $product->slug) }}" class="block w-full h-full">
                                        ...
                                        <div x-show="viewMode === 'grid'" class="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-20">
                                            <button wire:click="addToCart({{ $product->id }})" class="w-full bg-gray-900 text-white font-medium py-3 rounded-lg shadow-lg hover:bg-black flex items-center justify-center gap-2" type="button">
                                    ...
                                    <div class="p-4" :class="viewMode === 'list' ? 'w-2/3 pl-6' : ''">
                                        ...
                                        <h3 class="text-sm font-bold text-gray-900 leading-snug mb-2 group-hover:text-blue-600 transition-colors line-clamp-2">
                                            <a href="{{ route('website-v2.product.detail', $product->slug) }}">{{ $product->title }}</a>
                                        </h3>
                                        ...
                                        <div class="flex items-center mb-3">
                                            ...
                                            <span class="text-xs text-gray-400 ml-1">(12 reviews)</span>
                                        </div>

                                        <div class="flex items-center gap-3">
                                            @if ($product->sale_price > 0)
                                                <span class="text-lg font-extrabold text-blue-600">{{ number_format((float) $product->sale_price) }}</span>
                                                <span class="text-sm text-gray-400 line-through">{{ number_format((float) $product->regular_price) }}</span>
                                            @else
                                                <span class="text-lg font-extrabold text-gray-900">{{ number_format((float) $product->regular_price) }}</span>
                                            @endif
                                        </div>

                                        <div x-show="viewMode === 'list'" class="mt-4">
                                            <button wire:click="addToCart({{ $product->id }})" class="px-6 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-black transition" type="button">
                                                Add to cart
                                            </button>
                                        </div>
                                    </div>
    ...
    <div x-show="showToast"
        ...
        <div>
            ...
            <p class="text-sm text-gray-300">Product has been added to cart.</p>
        </div>
    </div>
</div>
