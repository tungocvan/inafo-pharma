<div class="bg-gray-50 min-h-screen py-12 font-sans">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-end justify-between mb-8">
            <div>
                <h1 class="text-3xl md:text-4xl font-black text-gray-900 tracking-tight">Cart</h1>
                <p class="text-gray-500 mt-2">
                    You have <span class="font-bold text-gray-900">{{ $this->cartData['items']->count() }}</span> products in your cart.
                </p>
            </div>
            <a href="{{ route('website-v2.home') }}" class="hidden md:flex items-center gap-1 text-sm font-bold text-blue-600 hover:text-blue-700 transition">
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        @if ($remaining > 0)
                            <p class="text-sm text-gray-600 mb-2">
                                Add <span class="font-bold text-red-500">{{ number_format($remaining) }}</span> more to unlock <span class="font-bold text-green-600 uppercase">free shipping</span>
                            </p>
                        @else
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 divide-y divide-gray-100 overflow-hidden">
                        @foreach ($this->cartData['items'] as $item)
                            <div class="p-6 flex gap-6 group transition hover:bg-gray-50/50 relative" wire:key="website-v2-cart-item-{{ $item->id }}">
                                ...
                                <div class="flex-1 flex flex-col justify-between">
                                        ...
                                        <div>
                                            <h3 class="font-bold text-gray-900 text-lg leading-snug hover:text-blue-600 transition">
                                                @if ($item->product?->slug)
                                                    <a href="{{ route('website-v2.product.detail', $item->product->slug) }}">
                                            ...
                                            <p class="text-sm text-gray-500 mt-1">Variant: Default</p>
                                        </div>
                                    ...
                                    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-4">
                                        ...
                                        <div class="text-right">
                                            ...
                                            @if ($item->quantity > 1)
                                                <p class="text-xs text-gray-500">{{ number_format($item->price) }} / item</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100">
                        ...
                        <div class="space-y-4 text-sm">
                            <div class="flex justify-between text-gray-600">
                                <span>Subtotal</span>
                                <span class="font-medium text-gray-900">{{ number_format($this->cartData['subtotal']) }}</span>
                            </div>
                            ...
                            @if ($this->cartData['discount'] > 0)
                                <div class="flex justify-between items-start text-green-600 bg-green-50 p-2 rounded-lg border border-green-100">
                                    ...
                                    <span class="font-bold">-{{ number_format($this->cartData['discount']) }}</span>
                                </div>
                            @else
                                <div class="flex justify-between text-gray-600">
                                    <span>Discount</span>
                            ...
                            <div class="flex justify-between text-gray-600">
                                <span>Shipping</span>
                                <span class="text-green-600 font-bold">Calculated at checkout</span>
                            </div>
                            ...
                            <div class="flex justify-between items-end">
                                ...
                                <span class="text-2xl font-black text-blue-600 tracking-tight">{{ number_format($this->cartData['total']) }}</span>
                            </div>

                            <p class="text-xs text-gray-400 text-right">(VAT included if applicable)</p>
                        </div>

                        <div class="mt-8">
                            <a href="{{ route('website-v2.checkout.index') }}"
                        ...
                        <div class="mt-6 flex flex-col items-center gap-3">
                            <p class="text-xs text-gray-400 font-medium">100% secure payment</p>
        ...
        @else
            <div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl shadow-sm border border-gray-100 text-center">
                ...
                <p class="text-gray-500 max-w-md mx-auto mb-8">Looks like you have not added any products yet. Explore our products and come back here.</p>
                <a href="{{ route('website-v2.product.list') }}" class="px-8 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition transform hover:-translate-y-1">
                    Shop now
                </a>
            </div>
        @endif
