<div class="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Danh mục thông tin chi tiết các sản phẩm dược phẩm và hồ sơ pháp lý
                trong hệ thống.</p>
        </div>
        <div>
            <a href="{{ route('admin.pharma.create') }}"
                class="inline-flex items-center justify-center rounded-xl px-4 py-3 font-semibold text-sm bg-blue-600 text-white hover:bg-blue-700 transition-colors shadow-sm gap-2">
            ...
            <div class="md:col-span-3">
                ...
                <div class="relative">
                    <input type="text" wire:model.live.debounce.300ms="search"
                        ...
                        class="w-full rounded-xl border border-gray-300 pl-11 pr-4 py-3 mt-1 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-shadow">
                    <div
                        class="absolute inset-y-0 left-0 pl-4 mt-1 flex items-center pointer-events-none text-gray-400">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody class="divide-y divide-gray-200 text-sm text-gray-700">
                    @forelse($medicines as $index => $medicine)
                        <tr class="hover:bg-gray-50/70 transition-colors">
                            ...
                            <td class="py-4 px-6">
                                <div class="font-semibold text-gray-900">{{ $medicine->name }}</div>
                                <div class="text-xs text-gray-500 mt-0.5">{{ $medicine->dosage_form }} •
                                    {{ $medicine->packaging_specification }}</div>
                            </td>
                            <td class="py-4 px-6">
                                <div class="font-medium text-gray-800 truncate max-w-xs"
                                    title="{{ $medicine->active_ingredients }}">
                                    {{ $medicine->active_ingredients }}
                                </div>
                                <div class="text-xs text-gray-500 mt-0.5">Hàm lượng: {{ $medicine->concentration }}
                                </div>
                            </td>
                            <td class="py-4 px-6">
                                <span
                            ...
                            <td class="py-4 px-6">
                                ...
                                <div class="text-xs text-gray-500 mt-0.5 truncate max-w-xs"
                                    title="{{ $medicine->manufacturing_company }}">
                                    {{ $medicine->manufacturing_company }}
                                </div>
                            </td>
                            <td class="py-4 px-6">
                                @if ($medicine->profile_link)
                                    <a href="{{ $medicine->profile_link }}" target="_blank"
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="inline-flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.pharma.edit', $medicine->id) }}"
                            ...
                            <td colspan="7" class="py-12 px-6 text-center">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2m0-5a7 7 0 1114 0 7 7 0 01-14 0z" />
                                    </svg>
                                    <p class="text-sm font-medium text-gray-900">Không tìm thấy kết quả</p>
                                    <p class="text-xs text-gray-500">Thử thay đổi từ khóa tìm kiếm hoặc kiểm tra lại bộ
                                        lọc của bạn.</p>
                                </div>
                            </td>
                        </tr>
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-xs text-gray-500">
                            Hiển thị từ <span class="font-medium text-gray-700">{{ $medicines->firstItem() }}</span>
                            đến <span class="font-medium text-gray-700">{{ $medicines->lastItem() }}</span> trong tổng
                            số <span class="font-medium text-gray-700">{{ $medicines->total() }}</span> bản ghi
                        </p>
                    </div>
                    <div>
                        <nav class="relative z-0 inline-flex rounded-xl shadow-sm -space-x-px"
                            aria-label="Pagination">
                            <button type="button" wire:click="gotoPage({{ $medicines->currentPage() - 1 }})"
