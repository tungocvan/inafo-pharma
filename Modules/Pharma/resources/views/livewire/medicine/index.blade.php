<div class="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 sm:p-6">
        <h3 class="text-sm font-semibold text-gray-800 mb-3 uppercase tracking-wider">Nhập dữ liệu thuốc từ file dữ liệu</h3>
        <form wire:submit="importData" class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div class="w-full sm:flex-1 relative">
                <div class="flex items-center w-full rounded-xl border border-gray-300 px-4 py-3 mt-1 bg-gray-50/50 hover:bg-gray-50 transition-colors cursor-pointer group">
                    ...
                    <span class="text-sm text-gray-600 truncate">
                        @if($importFile)
                            <span class="text-blue-600 font-medium">📋 Đang chọn: {{ $importFile->getClientOriginalName() }}</span>
                        @else
                            Nhấp chọn hoặc kéo thả tệp tin dữ liệu (.csv, .xlsx) vào đây
                        @endif
                    </span>
                </div>
                @error('importFile') <span class="text-xs text-rose-600 mt-1 block">{{ $message }}</span> @enderror
            </div>

            <button type="submit"
                    wire:loading.attr="disabled"
                    class="w-full sm:w-auto inline-flex items-center justify-center rounded-xl bg-gray-900 px-6 py-3 mt-1 font-semibold text-sm text-white hover:bg-gray-800 transition-colors shadow-sm disabled:opacity-50">
                <span wire:loading.remove wire:target="importFile, importData">Xác nhận nhập dữ liệu</span>
                <span wire:loading wire:target="importFile">Đang tải file...</span>
                <span wire:loading wire:target="importData">Đang phân tích xử lý...</span>
            </button>
        </form>
    </div>
